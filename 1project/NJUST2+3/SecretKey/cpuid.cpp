...
QString CpuId::get()
{
    ...
    if (get_cpu_id(cpu_id)){
        result = QString::fromStdString(cpu_id);
    }
    else if(get_cpu_id_by_asm(cpu_id)){
        result = QString::fromStdString(cpu_id);
    }
    else if(get_cpu_id_by_system(cpu_id)){
        result = QString::fromStdString(cpu_id);
    }
...
bool CpuId::get_cpu_id_by_asm(std::string &cpu_id)
{
    cpu_id.clear();
...
void CpuId::parse_cpu_id(const char *file_name, const char *match_words, std::string &cpu_id)
{
    cpu_id.c_str();
...
bool CpuId::get_cpu_id_by_system(std::string &cpu_id)
{
    cpu_id.c_str();
...
bool CpuId::get_cpu_id(std::string &cpu_id)
{
