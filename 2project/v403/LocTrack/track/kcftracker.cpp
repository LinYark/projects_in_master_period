...
KCFTracker::KCFTracker(bool hog, bool fixed_window, bool multiscale, bool lab)
{
	...
	output_sigma_factor = 0.125;	//高斯 Y 的带宽


	if (hog) {    // HOG  
				  // VOT  
		//interp_factor = 0.012;
		interp_factor = 0.25;		//更新权重
		sigma = 0.6;		//高斯核函数的带宽
		// TPAMI  
		//interp_factor = 0.02;  
		//sigma = 0.5;   
		cell_size = 4;	//HOG Bin的大小
		_hogfeatures = true;
...
// 基于当前帧更新目标位置  
cv::Rect KCFTracker::update(cv::Mat image,cv::Rect flashRoi)
{
	_roi = flashRoi;
	// 修正边界  
...
void KCFTracker::train(cv::Mat x, float train_interp_factor)
{
	...
	cv::Mat k = gaussianCorrelation(x, x);//做一些FFT的运算
	cv::Mat alphaf = complexDivision(_prob, (fftd(k) + lambda));//偏移

	_tmpl = (1 - train_interp_factor) * _tmpl + (train_interp_factor)* x;//将上一帧得到的特征与当前帧按比例合成
...
cv::Mat KCFTracker::gaussianCorrelation(cv::Mat x1, cv::Mat x2)
{
	...
	if (_hogfeatures) {
		...
		for (int i = 0; i < size_patch[2]; i++) 
		{
			...
			cv::mulSpectrums(fftd(x1aux), fftd(x2aux), caux, 0, true);//两个傅立叶频谱的每个元素的乘法
			caux = fftd(caux, true);
			rearrange(caux);
...
cv::Mat KCFTracker::getFeatures(const cv::Mat & image, bool inithann, float scale_adjust)
{
	...
	float cx = _roi.x + _roi.width / 2;//中心点
	...
	// 提取目标区域像素，超边界则做填充  
	cv::Mat FeaturesMap;
	cv::Mat z = RectTools::subwindow(image, extracted_roi, cv::BORDER_REPLICATE);//不能让检测框大于输入图，也不能小于0
	...
	if (_hogfeatures) {
		...
		FeaturesMap = RectTools::getGrayImage(z).clone();
