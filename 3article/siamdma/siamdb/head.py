...
from siamdb.util.xcorr import xcorr_fast, xcorr_depthwise,xcorr_depthwise_pad
...
class DepthwiseXCorr(nn.Module):
    ...
    def forward(self, kernel, search):
        ...
        return out
class DepthwiseXCorr_nopad(nn.Module):
    def __init__(self, in_channels, hidden, out_channels, kernel_size=3):
        super(DepthwiseXCorr_nopad, self).__init__()
        self.conv_kernel = nn.Sequential(
                nn.Conv2d(in_channels, hidden, kernel_size=kernel_size, bias=False),
                nn.BatchNorm2d(hidden),
                nn.ReLU(inplace=True),
...
class singgelDepthwiseBAN(BAN):
    def __init__(self, in_channels=256, out_channels=256, cls_out_channels=2, weighted=False):
        super(singgelDepthwiseBAN, self).__init__()
        self.cls = DepthwiseXCorr_nopad(in_channels, out_channels, cls_out_channels)
...
class singelBAN(BAN):
    def __init__(self, in_channels, cls_out_channels, weighted=False):
        super(singelBAN, self).__init__()
        self.weighted = weighted
        for i in range(len(in_channels)):
            self.add_module('box'+str(i+2), singgelDepthwiseBAN(in_channels[i], in_channels[i], cls_out_channels))
        if self.weighted:
            self.cls_weight = nn.Parameter(torch.ones(len(in_channels)))
...
# from siamban.models.neck import AdjustAllLayer
# import torch
...
# n = AdjustAllLayer(in_channels,out_channels)
# n =n.cuda()

# t1= n(t)
# s1 = n(s)

# b = MultiBAN( [256, 256, 256],2,True)
# b=b.cuda()
# c, l = b(t1, s1)
