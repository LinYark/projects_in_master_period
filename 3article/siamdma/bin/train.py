...
from siamdb.optimizer.optimizer import learnplan,learnstep,modeleval,modeltrain
...
def train():
    ...
    model ,current_epoch,cur_lr = loadmodelinstance.loadmodel_double_new(model)
    current_epoch,cur_lr  = 1,0.0015

    dbpath = '/media/hfc/HFCU/DATA_ALL/TRAIN/DB/'
    jsonpaths1 = [
        [dbpath+'GOT10K/self.json',2],   
        # [dbpath+'LaSOT/self.json',8],   
        # [dbpath+'GTOT/self.json',36],   
        # [dbpath+'rgbt174/self.json',45], 
        # [dbpath+'rgbt60/self.json',1],
        ]
    jsonpaths2 = [[dbpath+'GOT10KVAL/self.json',1]]
    jsonpaths3 = [[dbpath+'rgbt174/self.json',1]]
    jsonpaths4 = [[dbpath+'rgbt60/self.json',1]]

    #dataset
    train_dataloader = build_data_loader(jsonpaths1,alltimes =1,rgbt= True,batchsize =8)
    val_dataloader = build_data_loader(jsonpaths2,alltimes =5,rgbt= True,batchsize = 8)
    val_RGBT234_dataloader = build_data_loader(jsonpaths3,alltimes =5,rgbt= True,batchsize =8)
    val_GTOT_dataloader = build_data_loader(jsonpaths4,alltimes =5,rgbt= True,batchsize = 8)
    #learn_plan
    warmupEpoch,partTrainEpoch = 1,15
