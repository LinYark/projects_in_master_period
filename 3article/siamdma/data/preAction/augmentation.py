...
class Augmentation:
    ...
    def _blur_aug(self, image,kernel):
    ...
    def _shift_scale_aug(self, image, bbox, crop_bbox, size, random_scale_x,random_scale_y,random_sx,random_sy):
        im_h, im_w = image.shape[:2]

        # adjust crop bounding box
        crop_bbox_center = corner2center(crop_bbox)
        if self.scale:
            scale_x = (1.0 + random_scale_x * self.scale)
            scale_y = (1.0 + random_scale_y * self.scale)
        ...
        if self.shift:
            sx = random_sx * self.shift
            sy = random_sy * self.shift
    ...
    def __call__(self, image, bbox, size,random_scale_x,random_scale_y,random_sx,random_sy,blur=1, kernel=0):
        shape = image.shape
        crop_bbox = center2corner(Center(shape[0]//2, shape[1]//2,
                                         size-1, size-1))

        # shift scale augmentation
        image, bbox = self._shift_scale_aug(image, bbox, crop_bbox, size,random_scale_x,random_scale_y,random_sx,random_sy)
        ...
        # blur augmentation
        if self.blur > blur:
            image = self._blur_aug(image,kernel)
